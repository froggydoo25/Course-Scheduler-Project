...
BEGIN
    ...
    then 
        ...
        #Check Intructor is available or not
        select max(schedule_id) into n from Schedules;
		set i = 1;
...
CREATE PROCEDURE `insertSectionIntoDatabase`(
    ...
    in section_number varchar(5),
    in duration int(11),
    in start_date_in_session Date,
    in end_date_in_session Date,
    in course_abbreviation varchar(255),
    out result varchar(10)
)
begin 
	 ...
	 where s.start_date = start_date_in_session and s.end_date = end_date_in_session;
